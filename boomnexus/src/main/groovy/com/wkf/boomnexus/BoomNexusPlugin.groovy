...
class BoomNexusPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        project.plugins.apply(BoomExtraArchivePlugin)
        project.plugins.apply(MavenPlugin)
        project.plugins.apply(SigningPlugin)

        BoomNexusPluginExtension extension = project.extensions.create('nexus', BoomNexusPluginExtension)
    ...
    private void configureTasks(Project project, BoomNexusPluginExtension extension) {
        project.afterEvaluate {
            changeInstallTaskConfiguration(project, extension)
            addArchiveTaskToOutgoingArtifacts(project, extension, BoomExtraArchivePlugin.SOURCES_JAR_TASK_NAME)
            addArchiveTaskToOutgoingArtifacts(project, extension, BoomExtraArchivePlugin.TESTS_JAR_TASK_NAME)
            addArchiveTaskToOutgoingArtifacts(project, extension, BoomExtraArchivePlugin.JAVADOC_JAR_TASK_NAME)
        }
    }

    private void changeInstallTaskConfiguration(Project project, BoomNexusPluginExtension extension) {
    ...
    private void addArchiveTaskToOutgoingArtifacts(Project project, BoomNexusPluginExtension extension, String taskName) {
    ...
    private void configureSigning(Project project, BoomNexusPluginExtension extension) {
        project.afterEvaluate {
            if (extension.sign) {
                project.signing {
                    required {
    ...
    private void signPomForUpload(Project project, BoomNexusPluginExtension extension) {
    ...
    private void signInstallPom(Project project, BoomNexusPluginExtension extension) {
    ...
    private void configurePom(Project project, BoomNexusPluginExtension extension) {
    ...
    private void createPomsProjectProperty(Project project, BoomNexusPluginExtension extension) {
        project.afterEvaluate {
            project.ext.poms = []
    ...
    private void configureUpload(Project project, BoomNexusPluginExtension extension) {
        project.afterEvaluate {
